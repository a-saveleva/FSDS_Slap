...
We, Slap, pledge our honour that the work presented in this assessment is our own. Where information has been derived from other sources, we confirm that this has been indicated in the work. Where a Large Language Model such as ChatGPT has been used we confirm that we have made its contribution to the final submission clear.

Date: 17.12.2024

Student Numbers: 20017452, 24246337, 25000148, 24231390, 24242570.

## Brief Group Reflection

| What Went Well | What Was Challenging |
| -------------- | -------------------- |
| We identified the project scope early, and spent most of the time on interpretation, discussion and refinements to EDA   | Interpretation of residuals |
| We are seeing some impact which we expected to see | Cleaning and deduplicating the InsideAirbnb dataset, and dealing with limitations |

## Priorities for Feedback

Are there any areas on which you would appreciate more detailed feedback if we're able to offer it?

* Our approach to identifying valid airbnbs;
* Our approach to interpretation;
* Our suggestions for future STL regulation.
...
InsideAirbnb is a non-profit organisation founded and maintained by Murray Cox, a New York-based community activist, with contributions from John Morris, Taylor Higgins, and oversight from the Advisory Board [@insideairbnb].

## 2. Why did they collect the InsideAirbnb data?

The initial motivation was to address gaps in Airbnb's self-reported statistics, which may not fully reflect the platform's impact on housing [@Cox2016]. InsideAirbnb exposes abuses of power within the Airbnb model and promotes open, shared data for social justice [@Cox2023, @Cox2021, @insideairbnbDataPolicies].

## 3. How was the InsideAirbnb data collected?  

Data is scraped quarterly from Airbnb's public site using open-source tools - Python and PostgreSQL [@Tomslee2024]. The process involves verification, cleansing, analysis, and aggregation. These are the assumptions, since the scripts are unavailable.
...
The quality and completeness of the scraped dataset is determined by the openness of the platform, and the validity of the scraping. Both can’t be guaranteed. Inaccuracies include imprecise location, inconclusive calendar availability and pricing, and presence of redundant or duplicated listings [@insideairbnbDataAssumptions], among others [@Alsudais2020].

While anonymisation and partial redundancy of the snapshot data are expected, and the ridiculously high prices appearing in the dataset are a way to trick the algorithm [@GutHend2018], the latter two present a major limitation for research and, subsequently, policy-making related to STL (short-term lets). 

A unique case of data disclosure for London social housing fraud exists [@Hoult2022], but Airbnb generally prevents access to meaningful data and fails to block unlawful listings [@Tims2024]. It displays available dates, merging booked and unavailable into one category. As a result, availability is an unreliable metric. Hosts can keep calendars open year-round and close them near the 90-day limit, and it is unclear where they are on that scale.

The website features a 90-day counter to block further bookings, visible to hosts only [@airbnbRentHome]. There are many cases of hosts listing the same property multiple times, or both the entire home and individual rooms, to overcome this block. This could lead to over-counting and distorted estimates, pending further analysis.

## 5. What ethical considerations does the use of the InsideAirbnb data raise? 

Privacy concerns arise as aggregated data risks re-identification, especially in smaller areas or via cross-referencing, raising informed consent issues [@Ohm2009]. Biases in the data itself are also a concern, as the data may overrepresent certain neighbourhoods or property types, causing misleading conclusions [@Brenning2023, @insideairbnbDataAssumptions]. The authors' activist framing suggests a negative view of Airbnb’s effects. It raises reflexivity concerns: are biases fully acknowledged? Benefits to one group may harm others, including Airbnb hosts and related industries. 

Potential impact of research findings should also be considered. Highlighting STL benefits could worsen gentrification and housing shortages, and policy use of findings may disrupt housing markets - there is a discussion on how recently proposed changes to STL regulation might worsen the housing crisis [@londoncouncilsBoroughsShare].

To mitigate these issues, researchers must ensure transparency in their methods and consider the potential harm their findings could cause to local communities [@Prentice2023, @Alsudais2020].
...
We cleaned the InsideAirbnb data by defining active listings as those with recent reviews, removing null price and bed data as errors, and flagging listings available more than the London average [@onsHostsListings] as commercial. In 2023, 152,050 nights were booked across 111,880 STLs in London, averaging 5.84 nights per listing. We filtered out listings and hosts with “fair” platform use. Duplicates were detected using a tailored algorithm with 67-74% accuracy, validated via randomized sampling.
...
# Download data files from github repo from data folder and save these to the  
# same folder structure as the github repo
# ChatGPT was used to troubleshoot issues with building the function and adding
# file size checks into the workflow
#_____________________________________________________________________________________________________________________________

import os
import requests

# parameters for the github url
owner = "a-saveleva"  # github username
repo = "FSDS_Slap"  # repo name
base_folder = "data"  # starting folder
branch = "main"  # branch name, use 'main' as default
root_dir = os.getcwd()  # Use the script's current working directory
os.makedirs(root_dir, exist_ok=True) # ensure root directory exists

# build the url from the parameters
api_url = f"https://api.github.com/repos/{owner}/{repo}/contents/"

# create function to download all files within specified folder
def download_folders(folder_path):
    url = f"{api_url}{folder_path}?ref={branch}"
    ...
    if response.status_code == 200:
        ...
        for item in contents:
            if item["type"] == "file":
                ...
                local_path = os.path.join(root_dir, folder_path, item["name"])
                ...
                if os.path.exists(local_path):
                    ...
                    # if file exists then check that the file size is the same
                    if local_size == remote_size:
                        print(f"Skip {item['name']} as it already exists locally and is up-to-date.")
                        continue
                    else:
                        print(f"File {item['name']} exists but file size differs, re-downloading....")
                ...
                if file_response.status_code == 200:
                    ...
                    file_size = len(file_response.content) / 1024
                    print(f"Download complete: {item['name']} ({file_size:.2f} KB)")
                else:
                ...
                download_folders(subfolder_path)  # Fix: recursive call to itself
    else:
        print(f"Unable to retrieve folder {folder_path}: {response.status_code} {response.text}")

# call download_folders to start downloading data files
download_folders(base_folder)
print(f"All {base_folder} downloads complete!")
```
...
#_____________________________________________________________________________________________________________________________
# Download specific files by type that are required for the output
# Adapted from the download_folders code above which used ChatGPT 
# for troubleshooting
#_____________________________________________________________________________________________________________________________


def download_file_types(file_extensions):
    """
    Download specific file types from the root folder of the github repository.
    """
            ...
            if item["type"] == "file":
                ...
                if file_extension in file_extensions:
                    ...
                    if os.path.exists(local_path):
                        ...
                        if local_size == remote_size:
                            print(f"Skipping {file_name}, already exists locally and is up-to-date.")
                            continue
                        else:
                            print(f"File {file_name} exists but size differs, re-downloading.")
                    ...
                    if file_response.status_code == 200:
                        with open(local_path, "wb") as f:
                            f.write(file_response.content)
                        file_size = len(file_response.content) / 1024
                        print(f"Download complete: {file_name} ({file_size:.2f} KB)")
                    else:
                        print(f"Failed to download {file_name}: {file_response.status_code}")
    else:
        print(f"Failed to fetch root folder contents: {response.status_code} {response.text}")

# Define the file types to download
file_types = [".bib", ".csl"]

# Start downloading files
print(f"Starting download of {file_types} from root folder...")
download_file_types(file_types)
print(f"All {file_types} downloads complete!")
```
...
# Set the limits for the color scale using raw residual values
...
# Plot the residuals map without normalization
Rgdf.plot(
    column='residuals',
    cmap='magma_r',  # Matching the color scheme
    edgecolor='white',
    linewidth=0.8,
    vmin=vmin,       # Use raw minimum
    vmax=vmax,       # Use raw maximum
    ax=ax,
    legend=False     # Turn off default legend
)

# Add custom colorbar
sm = plt.cm.ScalarMappable(cmap='magma_r', norm=mpl.colors.Normalize(vmin=vmin, vmax=vmax))
sm._A = []  # Dummy array for colorbar
cbar = fig.colorbar(sm, ax=ax, orientation="vertical", fraction=0.02, pad=0.02, shrink=0.6)
cbar.set_label("Residual Value", fontsize=12)
cbar.outline.set_visible(False)  # Remove the colorbar border

# Set title and formatting
ax.set_title("Spatial Distribution of Residuals", fontsize=14, loc='left')
ax.set_axis_off()  # Hide axes for cleaner layout
