...
InsideAirbnb is a non-profit organisation founded and maintained by Murray Cox, a New York-based community activist, with contributions from John Morris, Taylor Higgins, and oversight from the Advisory Board (source).

## 2. Why did they collect the InsideAirbnb data?

The initial motivation was to address gaps in Airbnb's self-reported statistics, which may not fully reflect the platform's impact on housing (source). InsideAirbnb exposes abuses of power within the Airbnb model and promotes open, shared data for social justice (source1, source2, source3).

## 3. How was the InsideAirbnb data collected?  

Data is scraped quarterly from Airbnb's public site using open-source tools - Python and PostgreSQL (source). The process involves verification, cleansing, analysis, and aggregation. These are the assumptions, since the scripts are unavailable.
...
The quality and completeness of the scraped dataset is determined by the openness of the platform, and the validity of the scraping. Both can’t be guaranteed. Inaccuracies include imprecise location, inconclusive calendar availability and pricing, and presence of redundant or duplicated listings (source), among others (source).
While anonymisation and partial redundancy of the snapshot data are expected, and the ridiculously high prices appearing in the dataset are a way to trick the algorithm (source), the latter two present a major limitation for research and, subsequently, policy-making related to STL (short-term lets). 
A unique case of data disclosure for London social housing fraud exists (source), but Airbnb generally prevents access to meaningful data and fails to block unlawful listings (source). It displays available dates, merging booked and unavailable into one category. As a result, availability is an unreliable metric. Hosts can keep calendars open year-round and close them near the 90-day limit, and it is unclear where they are on that scale.
The website features a 90-day counter to block further bookings, visible to hosts only (source). There are many cases of hosts listing the same property multiple times, or both the entire home and individual rooms, to overcome this block. This could lead to over-counting and distorted estimates, pending further analysis.

## 5. What ethical considerations does the use of the InsideAirbnb data raise? 

Privacy concerns arise as aggregated data risks re-identification, especially in smaller areas or via cross-referencing, raising informed consent issues (source). Biases in the data itself are also a concern, as the data may overrepresent certain neighbourhoods or property types, causing misleading conclusions (Source), (Source). The authors' activist framing suggests a negative view of Airbnb’s effects. It raises reflexivity concerns: are biases fully acknowledged? Benefits to one group may harm others, including Airbnb hosts and related industries. 
Potential impact of research findings should also be considered. Highlighting STL benefits could worsen gentrification and housing shortages, and policy use of findings may disrupt housing markets - there is a discussion on how recently proposed changes to STL regulation might worsen the housing crisis (source).
To mitigate these issues, researchers must ensure transparency in their methods and consider the potential harm their findings could cause to local communities (source) (source).
...
We cleaned the InsideAirbnb data by defining active listings as those with recent reviews, removing null price and bed data as errors, and flagging listings available more than the London average (source) as commercial. In 2023, 152,050 nights were booked across 111,880 STLs in London, averaging 5.84 nights per listing. We filtered out listings and hosts with “fair” platform use. Duplicates were detected using a tailored algorithm with 67-74% accuracy, validated via randomized sampling.
...
# Download data files from gitbub repo to data folder and save these to the  
...
root_dir = ""  # folder to save files locally, "" is the root folder

# build the url from the parameters
api_url = f"https://api.github.com/repos/{owner}/{repo}/contents/"

# create function to download all files within data folder
def download_all_folders(folder_path):
    ...
    if response.status_code == 200:
        ...
        for item in contents:
            if item["type"] == "file":
                ...
                download_all_folders(subfolder_path)
    else:
        print(f"Unable to retrieve folder {folder_path}: {response.status_code} {response.text}")

# call download_all_folders to start downloading data files
download_all_folders(base_folder)
...
We identified 28,207 active London listings in 2024 - 55.22% year-on-year increase. The listings accounted for 0.48% and 0.74% of total dwellings respectively (source, source); entire homes - 0.35% and 0.58% respectively, in contrast to other sources (source1). The central areas exhibit the highest proportion of Airbnb listings relative to total dwellings, and this share increased progressively across these and adjacent boroughs from 2023 to 2024.
### Trend towards entire homes
In 2024, the percentage of entire homes on Airbnb increased to 78%, up from 73% in 2023. The share of private rooms decreased from 26% to 21%, indicating a trend towards more entire homes, which have a greater impact on housing. Some sources suggest STL ease and higher revenue may incentivize removing properties from long-term rental markets (source, source), even if they start STL as a private room. We therefore see the need to expand the methodology to include private rooms into the research scope.
...
We compare our observations with the Airbnb-endorsed studies (source1, source2). As seen from our EDA, the commercial use of the platform is increasing, and the impact on housing could be underestimated. We discuss it further in the next section.
...
Despite a narrowing rent-wage gap, rising living costs force low-income Londoners to spend up to 77% of wages on housing. Higher mortgage rates further exclude many from homeownership, disproportionately impacting vulnerable populations, leading to poverty, evictions, overcrowding, or homelessness (source, source, source, source, source, source, source, source). There are many claims that STLs exacerbate this housing crisis, pushing the demand down to the social and private rental sector.
In February 2024, the UK government announced reforms requiring planning permission for STLs exceeding 90 nights, and a mandatory STL registration scheme (source) that will close the STL data availability gap, discussed in Q4. Additionally, the furnished holiday lettings tax regime will be abolished (source). These continuous efforts aim to keep homes available for local communities.
Airbnb claims no link between listing growth and rising housing costs, backed by studies discussed in Q6. A caveat of published studies is their UK-wide statistics for some parameters, which may not reflect London’s heated STL market. With Airbnb comprising 47% of European platform STL supply and 30% across all types, STL impacts on housing may be more significant (source, source).
### Our research
We base our research on the independent studies conducted in 2019 and 2020 (source, source). Our goal is to identify proxies accounting for the impacts of the Airbnb listings on housing in London. We utilise the methodology outlined in the articles above.
### Datasets:
...
* % or N of Houses of Multiple Occupancy (source)
